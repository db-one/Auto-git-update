...
# 判断变量值，如果有效发送通知
if [ -n "$FOLDERS" ]; then  curl "https://api.telegram.org/bot$TELEGRAM_BOT_TOKEN/sendMessage" -d "chat_id=$TELEGRAM_CHAT_ID&text=🚫分支：$matrix_target ；源码同步失败，失败列表：$FOLDERSX  ......🈹"; else curl "https://api.telegram.org/bot$TELEGRAM_BOT_TOKEN/sendMessage" -d "chat_id=$TELEGRAM_CHAT_ID&text=🎉分支：$matrix_target ；源码同步成功，已同步到最新......💯"; fi
rm -rf FOLDERS.md
