...
# 判断变量值，如果有效发送微信通知
if [ -n "$FOLDERS" ]; then  curl -X POST "https://api.telegram.org/bot"${{ secrets.TELEGRAM_BOT_TOKEN }}"/sendMessage?chat_id="${{ secrets.TELEGRAM_CHAT_ID }}"&text=🎉 源码同步失败-Lienol-$FOLDERSX...... 😋"; else touch 同步完成; fi
rm -rf FOLDERS.md
