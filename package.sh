...
# 判断变量值，如果有效发送通知
#企业微信通知
if [ -n "$FOLDERS" ]; then  curl "http://$WECHAT_WORK_URL?token=$WECHAT_WORK_TOKEN&message=🚫源码同步失败，分支：Package_$matrix_target ；，失败列表：$FOLDERSX......"; else curl "http://$WECHAT_WORK_URL?token=$WECHAT_WORK_TOKEN&message=🎉源码同步成功，分支：Package_$matrix_target......"; fi
#TG通知
