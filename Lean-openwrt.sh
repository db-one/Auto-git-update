...
# 判断变量值，如果有效发送微信通知
if [ -n "$FOLDERS" ]; then  curl -k --data chat_id=$TELEGRAM_CHAT_ID --data "text=🎉 源码同步失败-lean-$FOLDERSX...... 😋" "https://api.telegram.org/bot"$TELEGRAM_BOT_TOKEN/sendMessage""; else touch 同步完成; fi
rm -rf FOLDERS.md
